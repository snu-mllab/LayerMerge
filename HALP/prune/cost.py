...
class CostCalculator:
    ...
    def __init__(self, layers, groups, pre_group, fmap_table, latency_lut_file, lut_bs):
        self._layers = layers
        self._groups = groups
        ...
        for group in self._groups:
            ...
            for layer_name in group:
                ...
                if pre_group_name is None:
                    ...
                    pre_active_neuron_num = (
                        torch.sum(group_mask[pre_group_name].data).cpu().item()
                    )
                else:
                    pre_active_neuron_num = self._layers[pre_group_name[0]].weight.size(
                        0
                    )
    ...
    def _get_layer_latency(self, cin, cout, kernel, feat_size, stride, groups):
        ...
        if key not in self._latency_lut:
            raise ValueError(
                f"Configuration of {key} does not exist in latency lookup table."
            )
        ...
        for group in self._groups:
            ...
            for layer_name in group:
                ...
                if pre_group_name is None:
                    ...
                    pre_active_neuron_num = int(
                        self._layers[pre_group_name[0]].weight.size(0)
                    )
                ...
                for i in range(channel_group_count):
                    ...
                    reduced_latency = self._get_layer_latency(
                        ...
                        if conv_groups == 1
                            ...
                            pre_active_neuron_num - channel_group_size * (i + 1), 0
                        ),
