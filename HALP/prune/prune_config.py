    ...
    def _set_prune_setting(self):
        ...
        for group in reg_groups:
            ...
            for item in group["layers"]:
                ...
                bn_name_list = (
                    item["bn_name"].replace(" ", "").split(",")
                    if item["bn_name"]
                    else ["" for _ in range(len(name_list))]
                )
                pre_conv_name_list = (
                    item["pre_conv"].replace(" ", "").split(",")
                    if item["pre_conv"]
                    else ["" for _ in range(len(name_list))]
                )
        ...
        for group_name, pre_conv_name in self._pre_group.items():
            self._pre_group[group_name] = (
                self._layer_to_group[pre_conv_name]
                if pre_conv_name in self._layer_to_group
                else None
            )
