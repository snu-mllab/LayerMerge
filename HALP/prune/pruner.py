...
class Pruner:
    def __init__(self, model, exp_cfg):
        ...
        self._prune_target = set_latency_prune_target(
            initial_latency, self._prune_start, self._prune_steps, exp_cfg.prune_ratio
        )
    ...
    def prune_step(self, global_step):
        pruned_num = 0
        if (
            self._prune_start <= global_step < self._prune_end
            and (global_step + 1 - self._prune_start) % self._prune_interval == 0
        ):
            logger.info("Performing pruning.")
            self._importance_calculator.get_average_importance()
            pruned_num = self._latency_target_pruning(
                target_latency=self._prune_target.pop(0)
            )
    ...
    def _latency_target_pruning(self, target_latency):
        ...
        adaptive_group_latency_change = (
            self._cost_calculator.get_group_latency_contribute(
