...
class BasicBlock(nn.Module):
    ...
    def __init__(
        ...
        if self.conv1 is None:
            self.conv2 = None
            self.add_bias = nn.Parameter(
                torch.zeros(1, conv2_cout, 1, 1), requires_grad=False
            )
        else:
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        if self.conv1 is None or self.conv2 is None:
            self.conv3 = None
            self.add_bias = nn.Parameter(
                torch.zeros(1, conv3_cout, 1, 1), requires_grad=False
            )
        else:
...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layer1 = self._make_layer(
            block, 1, layers[0], layer_num_dict=layer_num_dict
        )
        ...
        fc_cin = (
            layer_num_dict[f"layer4.{layers[-1]-1}.{last_conv_name}"]
