...
parser = argparse.ArgumentParser(description="Main script for running HALP algorithm.")
parser.add_argument(
    "--exp",
    type=str,
...
parser.add_argument(
    "--seed", type=int, default=None, help="Random seed used for np and pytorch."
)
parser.add_argument(
    "--pretrained", type=str, default=None, help="The path of the pretrained model."
)
...
parser.add_argument(
    ...
    type=str,
    default=None,
    help="ImageNet dataset root directory. If not set, will use the one in the config file.",
)
parser.add_argument("--no_prune", action="store_true")
parser.add_argument("--mask", type=str, default=None, help="The path of the mask file.")
parser.add_argument(
    "--depth_path",
    type=str,
    default=None,
...
def main():
    ...
    if is_main:
        train_writer = SummaryWriter(
            os.path.join(os.environ["OUTPUT_PATH"], "tf_event")
        )

    if args.depth_path:
        logger.info(f"Loading `depth_file` from : {args.depth_path}")
    ##### Model #####
    model = get_model(
        exp_cfg.arch,
        exp_cfg.class_num,
        exp_cfg.enable_bias,
        group_mask_file=args.mask,
    ...
    if exp_cfg.lr_schedule == "step":
        lr_scheduler = lr_step_policy(
            exp_cfg.learning_rate, [30, 60, 80], 0.1, exp_cfg.warmup
        )
    elif exp_cfg.lr_schedule == "cosine":
        lr_scheduler = lr_cosine_policy(
            exp_cfg.learning_rate, exp_cfg.warmup, exp_cfg.epochs
        )
    ...
    logger.info(
        "Best top1 accuracy: {}, top5 accuracy: {}".format(best_prec1, best_prec5)
    )
