...
def get_optimizer(model, exp_cfg, resume_optimizer_state_dict=None):
    ...
    logger.info(
        " ! Weight decay applied to parameters: {}".format(exp_cfg.weight_decay)
    )
    logger.info(
        " ! Weight decay applied to BN parameters: {}".format(exp_cfg.bn_weight_decay)
    )
    ...
    if resume_optimizer_state_dict is not None:
        optimizer.load_state_dict(
            {
                k.replace("module.", ""): v
                for k, v in resume_optimizer_state_dict.items()
            }
        )
    ...
    if exp_cfg.amp:
        ...
        model, optimizer = amp.initialize(
            ...
            loss_scale="dynamic"
            if exp_cfg.dynamic_loss_scale
            else exp_cfg.static_loss_scale,
        )
