...
def get_holdout(mode):
    """Returns a list of filename of a holdout validation data

    Written in "kim24layermerge/holdout_val.txt" file
    """
    assert mode in ["train", "val"]
    file_path = pkg_resources.resource_filename(
        "layer_merge.kim24layermerge", f"holdout_{mode}.txt"
    )
...
def make_dataset(
    ...
    if holdout != None:
        ...
        holdout_fnames = get_holdout(holdout)
        ...
        for root, _, fnames in sorted(os.walk(target_dir, followlinks=True)):
            for fname in sorted(fnames):
                ...
                if is_valid_file(path):
                    ...
                    if holdout == "train" and not fname in holdout_fnames:
                        continue
