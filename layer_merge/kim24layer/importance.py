...
def measure_imp(args, model, criterion):
    ...
    indices = [i for i in range(1, model.module.depth) if i not in irreducible_conv]
    full_indices = set(range(0, model.module.depth))

    print(f"Number of blocks are {len(indices)}.")
    ...
    for ind, index in enumerate(indices):
        ...
        print("")
        print(
            f"blk  | (index): {(index)} | {now}"
        )

        conv_ind = full_indices - {index}
...
def main():
    ...
    if args.arch == "resnet34":
        ...
        dataset = args.arch.split("_")[1]
        config = dataset2config(dataset)
        model: LayerDDPMModel = DDPMModel(config)
