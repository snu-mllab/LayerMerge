...
from layer_merge.models.ddpm_merged_layer import make_depth_layer_ddpm
from layer_merge.models.ddpm_merged_layer import make_depth_layer_ddpm_from_model
from layer_merge.models.ddpm_merged import make_depth_ddpm
from layer_merge.models.ddpm_merged import make_depth_ddpm_from_model
from layer_merge.models.ddpm_layer import make_layer_ddpm
from layer_merge.models.ddpm_layer import make_layer_ddpm_from_model
...
class Diffusion(object):
    ...
    def build_model(self):
        ...
        if args.depth_method and args.depth_path:
            state = torch.load(args.depth_path)
            if args.depth_method == "kim24layer":
                conv_ind = state["conv_ind"]
                model = make_layer_ddpm(conv_ind, config.data.dataset.lower())
            elif args.depth_method == "kim24layermerge":
                conv_ind = state["conv_ind"]
                act_ind = state["act_ind"]
                model = make_depth_layer_ddpm(act_ind, conv_ind, config.data.dataset.lower())
            elif args.depth_method == "kim23efficient":
                act_pos = state["act_pos"]
                merge_pos = state["merge_pos"]
                model = make_depth_ddpm(act_pos, merge_pos, config.data.dataset.lower())
            else:
        ...
        if args.from_pruned:
            assert args.depth_method and args.depth_path
            state = torch.load(args.depth_path)
            if args.depth_method == "kim24layer":
                conv_ind = state["conv_ind"]
                model = make_layer_ddpm_from_model(model, conv_ind)
            elif args.depth_method == "kim24layermerge":
                conv_ind = state["conv_ind"]
                act_ind = state["act_ind"]
                model = make_depth_layer_ddpm_from_model(model, act_ind, conv_ind)
            elif args.depth_method == "kim23efficient":
                act_pos = state["act_pos"]
                merge_pos = state["merge_pos"]
                model = make_depth_ddpm_from_model(model, act_pos, merge_pos)
            else:
                raise NotImplementedError()
